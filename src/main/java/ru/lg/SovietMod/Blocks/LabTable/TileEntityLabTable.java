...
public class TileEntityLabTable extends TileEntityLockableLoot implements ITickable
{
	...
	{
		return new ContainerLabTable(playerInventory, this, playerIn);
	}
	...
	public void update() {
		if (!this.world.isRemote && this.numPlayersUsing != 0 && (this.ticksSinceSync + pos.getX() + pos.getY() + pos.getZ()) % 200 == 0)
        {
            ...
            {
                if (entityplayer.openContainer instanceof ContainerLabTable)
                { 
                    if (((ContainerLabTable)entityplayer.openContainer).getChestInventory() == this)
                    {
                        ++this.numPlayersUsing;
                    }
                }
