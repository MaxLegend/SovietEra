...
public class BasicRotateXZBlock extends Block
{
	...
	public IBlockState getStateForPlacement(World worldIn, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int meta, EntityLivingBase placer)
	{
		...
		else
		{
			...
			{
				return iblockstate.withProperty(FACING, BasicRotateXZBlock.EnumOrientation.forFacings(EnumFacing.UP, placer.getHorizontalFacing()));
			}
			else
			{
				return iblockstate;
	...
	{
		return this.getDefaultState().withProperty(FACING, BasicRotateXZBlock.EnumOrientation.byMetadata(meta & 7));
	}

	public int getMetaFromState(IBlockState state)
	{
		int i = 0;
		i = i | ((BasicRotateXZBlock.EnumOrientation)state.getValue(FACING)).getMetadata();
