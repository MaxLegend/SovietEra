...
{	
	...
	private static AxisAlignedBB[] SIDE_AABB = new AxisAlignedBB[] {
			new AxisAlignedBB(0D, 0D, 0.3D, 1D, 1D, 1D),
			new AxisAlignedBB(0D, 0D, 0D, 1D, 1D, 0.7D),
			new AxisAlignedBB(0D, 0D, 0D, 0.7D, 1D, 1D),
			new AxisAlignedBB(1D, 0D, 0D, 0.3D, 1D, 1D)
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ)
	{
		...
		player.openGui(SovietCore.INSTANCE, GuiHandler.GUI_SAFE, world, pos.getX(), pos.getY(), pos.getZ());
		return true;
